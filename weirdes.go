// @Title           weirdes.go
// @Description     A weird DES enc/dec algorithm
// @Author          Liki4 2022/08/13
// @Update          Liki4 2022/08/15

// Package weirdes is a weird DES enc/dec algorithm written in go
package weirdes
...
// StrEnc
// Encrypt the plaintext to encrypted hex string
...
// StrDec
// Decrypt the encrypted hex string to plaintext
func StrDec(data, firstKey, secondKey, thirdKey string) string {
	...
	for i := 0; i < iterator; i++ {
		...
		var intByte = [64]int{}
		for j := 0; j < 64; j++ {
			intByte[j], _ = strconv.Atoi(strByte[j : j+1])
		}
		var decByte [64]int
		if firstKey != "" && secondKey != "" && thirdKey != "" {
			tempBt := intByte
...
func btToStr(byteData [64]int) string {
	var str string
	for i := 0; i < 4; i++ {
		var count = 0
		for j := 0; j < 16; j++ {
...
func dec(dataByte, keyByte [64]int) [64]int {
	keys := generateKeys(keyByte)
	ipByte := initPermute(dataByte)
	var ipLeft [32]int
	var ipRight [32]int
	...
	for i := 15; i >= 0; i-- {
		...
		var tempRight = xor32(pPermute(sBoxPermute(xor48(expandPermute(ipRight), key))), tempLeft)
		for n := 0; n < 32; n++ {
			ipRight[n] = tempRight[n]
		}
	}
